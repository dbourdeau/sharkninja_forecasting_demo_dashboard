...
from forecast_model import CallVolumeForecaster, train_test_split
...
def main():
    ...
    with st.spinner("Training robust SARIMAX model (this may take 1-2 minutes)..."):
        try:
            update_status("Initializing model...", 10)
            forecaster = CallVolumeForecaster(weekly_seasonality=True, yearly_seasonality=True)
            
            update_status("Preparing data...", 20)
            
            update_status("Fitting SARIMAX model - this is the slow part!", 30)
            
            # Show warning that this takes time
            import time
            start_time = time.time()
            warning_box = st.info("⏳ **Model is optimizing...** This should take about 5-10 seconds.")
            
            try:
                # Fit model
                forecaster.fit(train_df, changepoint_prior_scale=changepoint_prior, 
                              seasonality_prior_scale=seasonality_prior, verbose=False)
                
                elapsed = time.time() - start_time
                warning_box.success(f"✓ **Model training completed in {elapsed:.1f} seconds!**")
                
            except Exception as fit_error:
                elapsed = time.time() - start_time
                warning_box.error(f"⚠ **Training error after {elapsed:.1f} seconds:** {str(fit_error)[:200]}")
                raise
            
            update_status("Model training complete! Generating forecast...", 90)
            progress_bar.progress(100)
            
            # Show model info in expandable section
            if hasattr(forecaster, 'model_fitted') and forecaster.model_fitted is not None:
                with st.expander("📊 Model Diagnostics & Summary (click to view)", expanded=False):
                    st.success("✓ Model trained successfully!")
                    try:
                        model_summary = forecaster.model_fitted.summary()
                        st.text(str(model_summary))
                    except:
                        st.info("Model fitted but summary unavailable")
    ...
    # Generate forecast and metrics for all tabs (cached)
...
4. Review and adjust forecast weekly
    ...
    with tab1:
        st.header("Weekly Call Volume Forecast")
        
        # Generate forecast
        future_forecast = forecaster.forecast_future(periods=forecast_periods)
        
        # Plot
        fig = plot_forecast(train_df, future_forecast, 
                           title=f"Call Volume Forecast - Next {forecast_periods} Weeks")
        ...
        with col1:
            ...
            # Handle NaN/inf values in forecast before converting to int
            forecast_display = future_forecast.copy()
            
            # Get median for fallback if needed
            median_val = forecast_display['yhat'].median()
            if not np.isfinite(median_val) or median_val == 0:
                median_val = 600  # Default fallback
            
            # Clean yhat
            forecast_display['yhat'] = forecast_display['yhat'].replace([np.inf, -np.inf], np.nan)
            forecast_display['yhat'] = forecast_display['yhat'].fillna(median_val)
            forecast_display['yhat'] = forecast_display['yhat'].clip(lower=0)
            
            # Clean lower bound
            forecast_display['yhat_lower'] = forecast_display['yhat_lower'].replace([np.inf, -np.inf], np.nan)
            forecast_display['yhat_lower'] = forecast_display['yhat_lower'].fillna(forecast_display['yhat'] * 0.8)
            forecast_display['yhat_lower'] = forecast_display['yhat_lower'].clip(lower=0)
            
            # Clean upper bound
            forecast_display['yhat_upper'] = forecast_display['yhat_upper'].replace([np.inf, -np.inf], np.nan)
            forecast_display['yhat_upper'] = forecast_display['yhat_upper'].fillna(forecast_display['yhat'] * 1.2)
            forecast_display['yhat_upper'] = np.maximum(forecast_display['yhat_upper'], forecast_display['yhat_lower'])
            
            # Now safe to convert to int
            forecast_summary = pd.DataFrame({
                'Date': forecast_display['ds'].dt.strftime('%Y-%m-%d'),
                'Forecasted Volume': forecast_display['yhat'].astype(int),
                'Lower Bound': forecast_display['yhat_lower'].astype(int),
                'Upper Bound': forecast_display['yhat_upper'].astype(int)
            })
            st.dataframe(forecast_summary, use_container_width=True, height=400)
        
        with col2:
            st.subheader("Forecast Statistics")
            st.metric("Average Forecasted Volume", f"{future_forecast['yhat'].mean():.0f}")
            st.metric("Min Forecasted Volume", f"{future_forecast['yhat'].min():.0f}")
            st.metric("Max Forecasted Volume", f"{future_forecast['yhat'].max():.0f}")
            st.metric("Forecast Range", f"{future_forecast['yhat'].max() - future_forecast['yhat'].min():.0f}")
